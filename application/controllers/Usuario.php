...
class Usuario extends CI_Controller {
    ...
    public function login(){
        ...
        if(is_object($usuario) && !empty($usuario)){
            ...
            if(password_verify($pass, $hash)){
                ...
                $islogin = array("status"  => FALSE, "msg" => "La contraseña no es valida");
            }  
        } else{
   ...
   public function addUser(){
       ...
       $datauser = array();
       $response = array();
       $datauser['nombre'] = $this->input->get('nombre');
       $datauser['apellido'] = $this->input->get('apellido');
       $datauser['password'] = password_hash($this->input->get('password'), PASSWORD_BCRYPT, array("cost" => 10));
       $datauser['correo'] = $this->input->get('correo');
       $datauser['telefono'] = $this->input->get('telefono');
   ...
   public function deleteUser($id = 0){
    ...
    public function getuser(){
      $this->load->model('Usuario_model');
      $data = $this->Usuario_model->getUser();
  ...
  public function getuserbuid(){
      ...
      $this->load->model('Usuario_model');
      $data = $this->Usuario_model->getUserById($myid);
      header('Content-Type: application/json');
      echo json_encode($data);
  }
